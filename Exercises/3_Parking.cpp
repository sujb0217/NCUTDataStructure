...
int main()
{
    ...
    function<void(vector<Car> &, Car &)> enStack = [](vector<Car> &stk, const Car& car) { stk.emplace_back(car); };
    function<void(vector<Car> &)> deStack = [](vector<Car> &stk) { stk.pop_back(); };
//
    // Initialize queue;
    queue q;
    q.init();
    ...
    while (cin >> operation >> num >> time, operation || num || time)
    {
        if (!operation)
        {
            ...
            Car car(num, time, 0);
            // Push in stack
            if (static_cast<int> (stk1.size()) < capacity)
                enStack(stk1, car);
                // Push in queue
            else
                q.Push(car);
    ...
    auto printPair = [](const pif &pp) { printf("%d %.2f\n", pp.first, pp.second); };
