...
struct treeNode {
    ...
    treeNode (char c, treeNode* left, treeNode* right) : val(c), left(left), right(right) {}
};
class Solution {
public:
    ...
    treeNode* build(vector<char> inorder, vector<char> postorder, int il, int ir, int pl, int pr) {
        if (il > ir or pl > pr) {
            return nullptr;
        }
