        ...
        for (int i = 0; i < row; ++i) {
            ...
            for (int j = 0; j < col; ++j) {
                ...
                if (!(ch ^ '/') && j < col - 1) {
                    ...
                    if (!(args[i][j + 1] ^ '*')) {
                        for (j += 2; i < row; ++i) {
                            for (j = (j < col - 1 ? j : 0), col = static_cast<int> (args[i].size()); j < col - 1; ++j)
...
void goMatch(vector<bracket> argc) {
    ...
    for (auto &i: argc) {
        if (isLeftBracket(i.first))
            stack.Push(i);

            // is right bracket
        else {
            if (stack.isEmpty() || !isMatch(stack.Top(), i)) {
                cout << "without matching '" << i.first << "' at line " << i.second << endl;
                return;
            } else
                stack.Pop();
        }
    ...
    if (stack.isEmpty()) {
        ...
        cout << "without matching '" << stack.Top().first << "' at line " << stack.Top().second << endl;
    }
}
