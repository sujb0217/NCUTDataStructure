...
struct HuffTreeNode {
    ...
    explicit HuffTreeNode(HuffTreeNode* node) : num(node->num), val(node->val), flag(node->flag), left(node->left), right(node->right) {}
    HuffTreeNode(int x, int v, int f) : num(x), val(v), flag(f), left(nullptr), right(nullptr) {}
};
...
for (auto &huff : HuffNodes) {
    ...
    // sort(heap.begin(), heap.end(), [](HuffTreeNode* ha, HuffTreeNode* hb) {return ha->val < hb->val;});

    // encode Huffman Tree
    while (heap.size() > 1) {
        auto Min = min_element(heap.begin(), heap.end(), [](HuffTreeNode* ha, HuffTreeNode* hb) {return ha->val < hb->val;});
        auto lt = heap.at(Min - heap.begin()); heap.erase(Min);
        Min = min_element(heap.begin(), heap.end(), [](HuffTreeNode* ha, HuffTreeNode* hb) {return ha->val < hb->val;});
        auto rt = heap.at(Min - heap.begin()); heap.erase(Min);
