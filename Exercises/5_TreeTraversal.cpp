...
struct TreeNode {
    ...
    TreeNode(char ch, TreeNode* left, TreeNode* right) : val(ch), left(left), right(right) {}
};
...
class Tree {
  public:
    ...
    TreeNode* build(vector<char>& preorder, vector<char>& inorder, int pl, int pr, int il, int ir) {
        if (pl > pr) {
            return nullptr;
        }
